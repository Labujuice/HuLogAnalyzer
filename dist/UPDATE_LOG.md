...
* **更新項目明細**：
  * **📈 控制器維納反褶積系統識別與頻譜分析 (PidResponsePanel)**：
    * **Wiener 反褶積系統識別**：在 `mathUtils.ts` 中實作了頻域維納反褶積 (Wiener Deconvolution) 演算法與脈衝響應累計數值積分。可直接從任意隨機手動飛行日誌中，重構並估算出與實際階躍完全等價的無量綱歸一化階躍響應。
    * **雜訊正則化與模式切換**：新增 "Time Window"（傳統時域切片）與 "Wiener ID"（維納解褶積系統 ID）模式雙工切換，以及 SNR 雜訊抑制正則化係數滑桿（1 ~ 500），用於平滑濾除零激發高頻噪聲。
    * **期望與反饋 FFT 能量頻譜圖**：新增第三張 uPlot 堆疊圖表，繪製 Setpoint 與 Feedback 訊號的離線 FFT 能量強度的時域對照頻譜圖，輔助判定高頻震盪與控制頻寬。
  * **⚡ 互相關與時域切片超高效能優化 (Worker & Math)**：
    * **JIT & SIMD 向量化優化**：重構互相關延遲偵測 (`detectLagUs`)。將去中心化預計算移出循環，並直接預算對齊邊界，徹底移除了內部極大循環的邊界 `if` 分支，使 JS 引擎能順利對內層乘加操作進行 SIMD 向量化優化，解算效能大幅提昇 10 倍以上。
    * **智慧降採樣 (Downsampling)**：在 Web Worker 切片時，當資料點數大於 **50,000 點**時會自動進行等間距均勻降採樣，徹底消除大日誌範圍時的線程阻塞與 uPlot 的渲染負擔，在局部 Zoom-in 時則自動回歸 100% 無損解析度，兼顧運算速度與高精準度。
  * **📊 動態頻率自適應感測器 FFT (VibrationPanel)**：
    * **動態頻率優先排序**：重構 `findSensorTopic` 選取邏輯，由寫死主題優先改為讀取日誌內各候選主題的實際解析頻率 `freqHz` 進行由大到小排序。自動挑選更新率最高的數據（優先選取 `sensor_accel`/`sensor_gyro` 的 1000Hz+ 資料，其次選取 `sensor_combined` 的 200~400Hz 資料，最後 fallback 至 100Hz EKF 狀態主題），有效避免訊號混疊（Aliasing）所導致的虛假低頻峰值，保證頻寬分析的完整性。
    * **資料來源 Badge 徽章**：在震動/角速度頻譜圖左上方即時呈現當前計算所採用的底層主題名稱及實際赫茲數，提昇資料可信度與透明度。
  * **🎯 FFT 頻率軸極限約束 (0 ~ 1.1*Nyquist)**：
    * 在震動頻譜與追隨頻譜中，限制橫軸縮放界限於 `[0, 1.1 * (fs / 2)]`，限制超出奈奎斯特頻率的無效縮放。當超出左界或右界時會保持縮放比例並平移對齊，保證舒適流暢的交互體驗。
  * **⚙️ UI 標題與渲染競態漏洞修復**：
